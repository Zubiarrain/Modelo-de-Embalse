...
class CurvaGarantia:
    ...
    def __init__(self,paso_calculo,falla_buscada,nivel_max,rendimiento,nivel_min_limite=None) -> None:
        ...
        self.input_data = extract_excel_data("input_energia.xlsx")
        self.nivel_min = self._nivel_max
        ...
        if self._nivel_max > self.nivel_max_absoluto:
            raise Exception(f'Nivel máximo {self._nivel_max} no puede ser mayor al máximo absoluto {self.nivel_max_absoluto} ')
        if self.nivel_min < self.nivel_min_limite:
            raise Exception(f'Nivel máximo {self._nivel_max} no puede ser menor al límite {self.nivel_min_limite} ')
