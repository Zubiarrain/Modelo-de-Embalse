...
class CalculosFallas:
    ...
    def __init__(self,input_data,falla_buscada, nivel_min,nivel_max,rendimiento):
        # ------------ INIT VARS ------------------
    ...
    def calculo(self):
        ...
        variables = [self.nivel_min,self.nivel_max,self.falla_buscada,self.rendimiento]
        rango = [len(self.nivel_min),len(self.nivel_max),len(self.falla_buscada),len(self.rendimiento)]
        casos = max(rango)

        for i in range(casos):
            datos=[self.nivel_min[0],self.nivel_max[0],self.falla_buscada[0],self.rendimiento[0]]
            ...
            info = f"NM= {calculo_falla.calculo_total.nivel_max} - Nm= {calculo_falla.calculo_total.nivel_min} - P= {calculo_falla.calculo_total.potencia_firme} - Déficit= {round(calculo_falla.calculo_total.falla,2)} "
