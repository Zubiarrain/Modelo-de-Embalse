...
class CalculoFalla:
    ...
    def __init__(self,falla_buscada, nivel_min,potencia_firme, rendimiento) -> None:
        # ----------------- INITIAL VARS -----------------------
        ...
        self.cambio_multiplicador = 0.25
    ...
    def _ajuste_falla(self):
        ...
        if diferencia_porcentual >= self.diferencia_porcentual_paso_previo and self.multiplicador > self.cambio_multiplicador:
            ...
            if self.falla_obtenida_exacta != 0:
                # Cuando estemos en falla nula, la diferencia porcentual será 1 para varias iteraciones hasta llegar a un valor de falla. En este rango no queremos que se modifique el multiplicador
                self.multiplicador -= self.cambio_multiplicador
    ...
    def _calcular_falla(self):
        while True:
            ...
            if self.nivel_max < self.nivel_min:
                ...
                # Sin embargo puede pasar que tengamos un cambio demasiado bruzco por ejemplo buscamos una falla del 5%, con un nivel minimo de 140. Imaginemos que en un paso de cálculo llegamos a una falla del 4.9% y nuestro cambio es de 5m, por lo que nuestro nuevo nivel máximo es 138<nivel minimo. Cuando pase por esta verificación entendería que se llegó a una falla máxima sin probar con valores intermedios entre 143 y 140. De suceder este caso por primera vez en las iteraciones del nivel maximo, este se ajusta al nivel minimo y el multiplicador del ajuste se lleva al mínimo para prevenir errores de converencia y se vuelve a iterar.
                if self.nivel_maximo_critico:
                    falla = round(self.falla_paso_previo,2)
                    print(f'La máxima falla para una potencia firme de {self.potencia_firme} es: {falla} ')
                    break
                else:
                ...
                if self.nivel_maximo_absoluto_alcanzado:
                    falla = round(self.falla_paso_previo,2)
                    print(f'Para una potencia firme de {self.potencia_firme} se alcanza una falla de {falla} llegando al nivel maximo absoluto de {self.nivel_max_absoluto}' )
                    self.error = True
                    break
                else:
            ...
            if self.falla_obtenida_exacta == self.falla_paso_previo and self.falla_obtenida_exacta != 0:
                ...
                print(f'La mínima falla para una potencia firme de {self.potencia_firme} es: {self.falla_obtenida} ')
                self.error = True
                break
            ...
            if self.falla_obtenida != self.falla_buscada:
                ...
                print('Potencia Firme:',self.potencia_firme)
                print('Falla Obtenida:',self.falla_obtenida)
                print('Nivel Máximo:',self.nivel_max)
