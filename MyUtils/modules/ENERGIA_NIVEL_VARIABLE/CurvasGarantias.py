...
class CurvasGarantias:
    ...
    def __init__(self,paso_calculo,falla_buscada,rendimiento,nivel_min_limite=[None]):
        # ------------ INIT VARS ------------------
    ...
    def calculo(self):
        ...
        variables = [self.paso_calculo,self.nivel_min_limite,self.falla_buscada,self.rendimiento]
        rango = [len(self.paso_calculo),len(self.nivel_min_limite),len(self.falla_buscada),len(self.rendimiento)]
        casos = max(rango)

        for i in range(casos):
            datos=[self.paso_calculo[0],self.nivel_min_limite[0],self.falla_buscada[0],self.rendimiento[0]]
            ...
            info = f"NM= {curva_garantia.nivel_max} - Déficit= {curva_garantia._falla_buscada} "
