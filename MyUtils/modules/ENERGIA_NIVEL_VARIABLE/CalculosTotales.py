...
class CalculosTotales:
    ...
    def __init__(self,input_data, nivel_min, potencia_firme, rendimiento):
        # ------------ INIT VARS ------------------
    ...
    def calculo(self):
        ...
        variables = [self.nivel_min,self.potencia_firme,self.rendimiento]
        rango = [len(self.nivel_min),len(self.potencia_firme),len(self.rendimiento)]
        casos = max(rango)

        for i in range(casos):
            datos=[self.nivel_min[0],self.potencia_firme[0],self.rendimiento[0]]
            ...
            resultado = [
                    ...
                    "Nivel Máximo [m]: " + str(calculo_total.nivel_max_mayor),
                    "Días del modelo: " + str(calculo_total.total_time)
                    ]
            info = f"NM= {calculo_total.nivel_max_mayor} - Nm= {calculo_total.nivel_min} - P= {calculo_total.potencia_firme} - Déficit= {round(calculo_total.falla,2)} "
